...
func Initdb() {
	...
	ALTER TABLE articles ADD FOREIGN_KEY(art_by) REFERENCE users(user_id) ON DELETE RESTRICT ON UPDATE CASCADE
	ALTER TABLE comments ADD FOREIGN_KEY(comment_by) REFERENCE users(user_id) ON DELETE RESTRICT ON UPDATE CASCADE
